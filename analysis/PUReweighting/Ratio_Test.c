...
int main(int argc, char* argv[]) {
  ...
  TFile* dataFile=TFile::Open("pu_truth_data2016_100bins.root"); 
  TFile* mcFile = TFile::Open("computed_mc_GJET_Pythia_pu_truth_100bins.root");

  TH1* dataHisto = static_cast<TH1*>(dataFile->Get("pileup"));
  TH1* mcHisto = static_cast<TH1*>(mcFile->Get("pileup"));

  TFile f1("../tuples/GJET_Pythia/PhotonJet_2ndLevel_GJet_Pt-15To6000_TuneCUETP8M1-Flat_13TeV_pythia8__25ns_ReReco_2016-05-31.root ");        
  ...
  dataHisto -> SetTitle("");
  dataHisto -> SetXTitle("n true interactions");
  dataHisto -> SetYTitle("Normalized");
  dataHisto-> SetLineColor(kBlue);
  dataHisto-> GetXaxis()->SetRangeUser(0,40);
