...
int main(int argc, char* argv[]) {
  ...
  double x1 = 1.305;
  printf("Valore di   %02.0f",x1*10);
  double x2 = 1.93;
  printf("Valore di   %02.0f",x2*10);
  double x3 = 2.5;
  printf("Valore di   %02.0f",x3*10);
  double x4 = 2.964;
  printf("Valore di   %02.0f",x4*10);
  double x5 = 3.2;
  printf("Valore di   %02.0f",x5*10);
  double x6 = 5.191;
  printf("Valore di   %02.0f",x6*10);


  TLine* line_one = new TLine(0.05, 1., 0.4, 1.);
  TLine* line_plus = new TLine(0.05, 1.05, 0.4, 1.05);
  TLine* line_minus = new TLine(0.05, 0.95, 0.4, 0.95);
  line_one -> SetLineColor(kRed);
  line_plus -> SetLineStyle(2);
  line_minus -> SetLineStyle(2);

  TGraphErrors* gr_respRatio_vs_alphacut = new TGraphErrors(0);
  gr_respRatio_vs_alphacut->SetName("response_vs_alphacut");

  //  double FitValue = 1.2;
  //  double FitError = 0.1;

    gr_respRatio_vs_alphacut->SetPoint(0, 0.10, 0.968);
    gr_respRatio_vs_alphacut->SetPointError(0, 0., 0.004 );

    gr_respRatio_vs_alphacut->SetPoint(1, 0.15, 0.975);
    gr_respRatio_vs_alphacut->SetPointError(1, 0., 0.004 );

    gr_respRatio_vs_alphacut->SetPoint(2, 0.20, 0.977);
    gr_respRatio_vs_alphacut->SetPointError(2, 0. , 0.004 );
  
    gr_respRatio_vs_alphacut->SetPoint(3, 0.25, 0.973);
    gr_respRatio_vs_alphacut->SetPointError(3, 0., 0.004 );

    gr_respRatio_vs_alphacut->SetPoint(4, 0.30, 0.972);
    gr_respRatio_vs_alphacut->SetPointError(4, 0., 0.004 );

    TCanvas* c1 = new TCanvas("c1", "c1", 800, 800);
    gr_respRatio_vs_alphacut->SetMarkerStyle(20);
    gr_respRatio_vs_alphacut->SetMarkerSize(1.5);
    gr_respRatio_vs_alphacut->SetMarkerColor(kBlue - 6);
    gr_respRatio_vs_alphacut->SetTitle(" ");
    gr_respRatio_vs_alphacut->GetXaxis()->SetTitle("#alpha cut");
    gr_respRatio_vs_alphacut->GetYaxis()->SetTitle("Data/MC");
    gr_respRatio_vs_alphacut->GetYaxis()->SetTitleOffset(1.5);
    TAxis *axis = gr_respRatio_vs_alphacut->GetXaxis();
    axis->SetLimits(0.05, 0.4);                 // along X
   gr_respRatio_vs_alphacut->GetHistogram()->SetMaximum(1.1);   // along         
   gr_respRatio_vs_alphacut->GetHistogram()->SetMinimum(0.7);  //   Y 
   gr_respRatio_vs_alphacut->SetLineColor(kBlue - 6);
   gr_respRatio_vs_alphacut->Draw("AP");    
   line_one->Draw("same");
   line_plus->Draw("same");
   line_minus->Draw("same");
   c1->SaveAs("Balancing_respRatio_vs_alphacut.png");


  TGraphErrors* gr_respRatioMPF_vs_alphacut = new TGraphErrors(0);
  gr_respRatioMPF_vs_alphacut->SetName("response_vs_eta");

  //  double FitValue = 1.2;
  //  double FitError = 0.1;

    gr_respRatioMPF_vs_alphacut->SetPoint(0, 0.10, 0.944);
    gr_respRatioMPF_vs_alphacut->SetPointError(0, 0., 0.004 );

    gr_respRatioMPF_vs_alphacut->SetPoint(1, 0.15, 0.954);
    gr_respRatioMPF_vs_alphacut->SetPointError(1, 0., 0.004 );

    gr_respRatioMPF_vs_alphacut->SetPoint(2, 0.20, 0.954);
    gr_respRatioMPF_vs_alphacut->SetPointError(2, 0. , 0.004 );
  
    gr_respRatioMPF_vs_alphacut->SetPoint(3, 0.25, 0.957);
    gr_respRatioMPF_vs_alphacut->SetPointError(3, 0., 0.004 );

    gr_respRatioMPF_vs_alphacut->SetPoint(4, 0.30, 0.958);
    gr_respRatioMPF_vs_alphacut->SetPointError(4, 0., 0.004 );


    TCanvas* c2 = new TCanvas("c2", "c2", 800, 800);
    gr_respRatioMPF_vs_alphacut->SetMarkerStyle(20);
    gr_respRatioMPF_vs_alphacut->SetMarkerSize(1.5);
    gr_respRatioMPF_vs_alphacut->SetMarkerColor(kBlue - 6);
    gr_respRatioMPF_vs_alphacut->SetTitle(" ");
    gr_respRatioMPF_vs_alphacut->GetXaxis()->SetTitle("#alpha cut");
    gr_respRatioMPF_vs_alphacut->GetYaxis()->SetTitle("Data/MC");
    gr_respRatioMPF_vs_alphacut->GetYaxis()->SetTitleOffset(1.5);
    TAxis *axis1 = gr_respRatioMPF_vs_alphacut->GetXaxis();
    axis1->SetLimits(0.05, 0.4);                 // along X
   gr_respRatioMPF_vs_alphacut->GetHistogram()->SetMaximum(1.1);   // along         
   gr_respRatioMPF_vs_alphacut->GetHistogram()->SetMinimum(0.7);  //   Y 
   gr_respRatioMPF_vs_alphacut->SetLineColor(kBlue - 6);
   gr_respRatioMPF_vs_alphacut->Draw("AP");    
   line_one->Draw("same");
   line_plus->Draw("same");
   line_minus->Draw("same");
   c2->SaveAs("MPF_respRatio_vs_alphacut.png");
